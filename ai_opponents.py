...
def student_ai_choose_move(state):
    ...
    tolerance = state.paddle_height // 6

    # Branch A: Ball is moving toward you → DEFEND
    ...
    # Branch B: Ball is moving away → REPOSITION TO CENTER
    else:
        if paddle_center < screen_center - tolerance:
            return MOVE_DOWN
        elif paddle_center > screen_center + tolerance:
            return MOVE_UP
        else:
            return MOVE_STAY
